...
class Libro:
    ...
    def listar_borrow():
        conn = Conexion()
        conn.query("SELECT p.identifier, book, user_id, name, date, date_of_return from borrows as b inner join books as p on b.book_id=p.identifier inner join users as u on b.user_id = u.identifier WHERE date_of_return IS NULL")
        response = conn.cursor.fetchall()
        for r in response:
